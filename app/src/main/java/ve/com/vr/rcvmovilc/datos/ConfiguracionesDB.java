...
public class ConfiguracionesDB {
    ...
    public boolean mfValidarExistencia(String vpCod, String vpVal){
        boolean vBand = false;

        Configuracion vConfig =  this.buscarConfiguracion(vpCod);

        if (vConfig != null) {
            if ("A".equals(vConfig.getEs_cf()) && vConfig.getVl_cf() == vpVal)
            {
                vBand= true;
            }
        }
